...
SELECT e.FirstName AS "First Name", e.LastName AS "Last Name", Sum(s.Quantity) AS Quantity FROM bestbuy.employees AS e
...
SELECT c.Name AS Category, d.Name AS Department FROM bestbuy.categories AS c
INNER JOIN departments AS d ON c.DepartmentID=d.DepartmentID
WHERE c.Name = 'Appliances' OR c.Name = 'Games';
/* joins: find the product name, total # sold, and total price sold,
 for Eagles: Hotel California --You may need to use SUM() */
SELECT p.Name AS Product, SUM(s.Quantity) AS Quantity, (s.PricePerUnit*SUM(s.Quantity)) AS Total FROM bestbuy.products AS p
